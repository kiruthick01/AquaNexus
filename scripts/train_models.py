...
``dissolved_oxygen``
    ...
    **Labels are real measurements.** This is the honest model - R2 ~0.39 under
    grouped cross-validation, which is barely ahead of assuming no change since
    the previous sample.
...
CAVEATS = {
    "dissolved_oxygen": [
        ...
        "Under-predicts oxygen by about 2.3 mg/L at low flow (< ~2 m3/s), which "
        "is where oxygen stress matters most. Low-flow predictions are "
        "unreliable.",
        "Predictions span 4.2-10.2 mg/L against an observed 3.0-17.0, so the "
        "model cannot reach the extremes it exists to catch.",
    ],
    "hsi": [
